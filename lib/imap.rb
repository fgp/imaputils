...
module Net
  ...
  class IMAP
    ...
    # Sends a APPEND command to append the +messages+ to the end of
    # the +mailbox+. You have to pass an array of messages to this
    # method. Each message is itself a three-element array, containing
    # the literal message, the flags, and the date_time of the message.
    # The flags and the date_time may be set to nil, or may be omitted
    # from the array.
    #
    # A Net::IMAP::NoResponseError is raised if the mailbox does
    # not exist (it is not created automatically), or if the flags,
    # date_time, or message arguments contain errors.
    def multiappend(mailbox, msgs)
      args = []
      msgs.each do |msg|
        txt, flags, date_time = *msg
        args.push(flags) if flags
        args.push(date_time) if date_time
        args.push(Literal::new(txt))
      end
      send_command("APPEND", mailbox, *args)
    end
    ...
    #        "UIDVALIDITY", "UNSEEN", "HIGHESTMODSEQ". Each value is a number.
    # 
    ...
    # Net::IMAP::ModSeq respresents the modseq part of a fetch response
    #
    # === Fields:
    #
    # modseq:: Modseq value for the message
    #
    # meta: List of ModSeqMeta Structures for metadata modseq values
    #
    ModSeq = Struct::new(:modseq, :meta)
    ...
    # modseq:: Modseq value for the message
    #
    # item: The item the modseq value applies to
    #
    # type: The item's type
    #
    ModSeqMeta = Struct::new(:modseq, :item, :type)
...
(?# 1:  TEXT    )([^\x00\r\n\]]*))/ni
      ...
      def modseq_data
        ...
        match(T_SPACE)
        match(T_LPAR)
        data = ModSeq::new(number, [ModSeqMeta::new])
        while true
          token = lookahead
          case token.symbol
            when T_RPAR then
              ...
              return name, data
            when T_STRING
              if !data.meta.last.item
                data.meta.last.item = string
              elsif !data.meta.last.type
                data.meta.last.type = string
              else
                parse_error("Unexpected token %s (%s)", token.symbol.to_s, token.value)
              end
            when T_NUMBER
              data.meta.last.modseq = number
              data.meta << ModSeqMeta::new
          end
        end
        return name, atom
      end
      ...
      def resp_text_code
        ...
        when /\A(?:UIDVALIDITY|UIDNEXT|UNSEEN|HIGHESTMODSEQ)\z/n
          match(T_SPACE)
  ...
  begin
    ...
    while true
      ...
      begin
        ...
        when "flags"
          if args[0]
            data = imap.fetch(args[0].to_i, ["FLAGS"])[0]
            puts data.attr["FLAGS"].join(", ").to_s
          else
            puts "missing argument"
          end
